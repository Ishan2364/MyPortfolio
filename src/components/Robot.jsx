...
const Robot = () => {
  ...
  const plasticMaterial = useMemo(() => new THREE.MeshPhysicalMaterial({
    // Darker Grey so it is clearly visible on white backgrounds
    color: '#94a3b8', 
    roughness: 0.2,
    metalness: 0.1,
    clearcoat: 1.0,
  ...
  useFrame((state, delta) => {
    ...
    if (bodyRef.current && headGroupRef.current) {
      ...
      headGroupRef.current.rotation.z = Math.sin(t * 1.5) * 0.02;
  ...
  return (
    <Float speed={2} rotationIntensity={0.2} floatIntensity={0.5}>
      {/* Reduced Scale slightly from 1.7 to 1.5 */}
      <group 
      ...
      >
        ...
        {/* FIXED: Arms moved closer (0.62 instead of 0.72) */}
        <group ref={leftShoulderRef} position={[-0.62, -0.3, 0]}>
        ...
        <group ref={rightShoulderRef} position={[0.62, -0.3, 0]}>
            <mesh position={[0, -0.25, 0]}>
                <capsuleGeometry args={[0.09, 0.5, 4, 16]} />
                <primitive object={plasticMaterial} attach="material" />
            </mesh>
